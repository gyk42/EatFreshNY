      ...
      if let email = emailTextField.text, let password = passwordTextField.text,
         ...
         {(user, error) in
            if let error = error {
               ...
               self.ref.child("users").updateChildValues(["\(currentUserID)":["userRole": "client", "userEmail": email, "userLocation" : "0", "userName": name, "userPhoneNumber": phone, "userWebsite": "0"]])
               
               UserModel.shared.user = User(userID: currentUserID, userRole: "client", userEmail: email, userLocation: "0", userName: name, userPhoneNumber: phone, userWebsite: "0", userLogo: "0")
